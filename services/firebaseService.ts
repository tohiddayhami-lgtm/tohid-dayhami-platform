...
// ── Meta Bazaars (named exhibitions/directories of shops) ──
export const saveMetaBazaarToCloud = async (bazaar: MetaBazaar) => {
    await setDoc(doc(db, "metaBazaars", bazaar.id), sanitizeData(bazaar));
    logSystemAction('UPDATE', 'MetaBazaar', `بازارچه ${bazaar.titleFa || bazaar.titleEn || bazaar.slug} ذخیره شد`, 'Master', bazaar.id);
};

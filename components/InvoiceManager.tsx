...
import { Invoice, InvoiceItem, Currency, InvoiceTemplate, Customer, Personnel, AppConfig } from '../types';
...
// Invoice number generator → e.g. SVC-2026-0609-01275
const genInvoiceNumber = (template: InvoiceTemplate, type: string, count: number): string => {
  const prefix = (template.invoicePrefix || (type ? type.slice(0, 3).toUpperCase() : 'SVC')) || 'SVC';
  const now = new Date();
...
const emptyDraft = (config: AppConfig, issuedBy: string, count: number): Invoice => {
  ...
  return {
    id: `inv-${Date.now()}`,
    number: genInvoiceNumber(tpl, type, count),
...
export const InvoiceManager: React.FC<Props> = ({ invoices, customers, config, currentUser, lang, onSaveInvoice, onDeleteInvoice, onUpdateConfig, readonly = false }) => {
  ...
  // ── Calculations (supports VAT-inclusive like the official invoice) ──
  const recompute = (inv: Invoice): Invoice => {
    ...
    if (inv.vatInclusive) {
      ...
      total = subTotal - (inv.discount || 0);
    } else {
      taxAmount = (subTotal * rate) / 100;
      total = subTotal + taxAmount - (inv.discount || 0);
    }
    return { ...inv, subTotal, taxAmount, total };
  };
  const netAmount = (inv: Invoice) => inv.vatInclusive ? inv.subTotal - inv.taxAmount : inv.subTotal;

  const startNew = () => { setDraft(emptyDraft(config, currentUser.fullName, invoices.length)); setMode('editor'); };
  const startEdit = (inv: Invoice) => { setDraft({ ...inv }); setMode('editor'); };
      ...
      {mode === 'company' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2 border-t border-gray-100 pt-4 mt-1"><h4 className="font-bold text-gray-700 text-sm">{lang === 'fa' ? 'اطلاعات بانکی (PAYMENT DETAILS)' : 'Bank / Payment Details'}</h4></div>
            ...
            <div className="md:col-span-2 border-t border-gray-100 pt-4 mt-1"><h4 className="font-bold text-gray-700 text-sm">{lang === 'fa' ? 'پیش‌فرض‌ها' : 'Defaults'}</h4></div>
            ...
            <div className="flex items-end gap-4">
              <label className="flex items-center gap-2 text-sm text-gray-700 pb-2"><input type="checkbox" className="w-4 h-4 accent-indigo-600" checked={companyForm.vatInclusive ?? true} onChange={e => setCompanyForm(f => ({ ...f, vatInclusive: e.target.checked }))} />{t.vatInc}</label>
      ...
      {mode === 'editor' && draft && (
        <div>
          ...
          <div className="bg-white mx-auto rounded-lg border border-gray-100 shadow-sm invoice-content text-gray-800" style={{ maxWidth: 800, padding: '40px 44px' }} dir="ltr">
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                ...
                <div className="mt-3 text-[12px] space-y-0.5">
                  ...
                  <div><span className="text-gray-500">Type </span><span className="font-medium">{draft.type || 'Services'}</span></div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border border-gray-200 rounded-md p-3">
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-bold tracking-wider text-gray-400 mb-1">BILL TO</p>
                  {!readonly && (
                    <div className="relative print:hidden">
                      <button type="button" onClick={() => setShowCustomerPicker(v => !v)} className="text-[10px] flex items-center gap-1 px-2 py-1 rounded border border-gray-200 text-gray-500 hover:bg-gray-50"><IconUsers className="w-3 h-3" />{t.pickCustomer}</button>
                      {showCustomerPicker && (
                        <div className="absolute left-0 top-full mt-1 w-72 bg-white border border-gray-200 rounded-xl shadow-xl z-20 p-2 text-right" dir={lang === 'fa' ? 'rtl' : 'ltr'}>
                          <input autoFocus value={customerSearch} onChange={e => setCustomerSearch(e.target.value)} placeholder={t.search} className="w-full px-3 py-1.5 rounded-lg border border-gray-200 text-sm outline-none mb-2" />
                          <div className="max-h-60 overflow-y-auto divide-y divide-gray-50">
                            {filteredCustomers.length === 0 ? <p className="text-xs text-gray-400 py-3 text-center">{t.noResult}</p> : filteredCustomers.map(c => (
              ...
              <div className="border border-gray-200 rounded-md p-3">
                <p className="text-[10px] font-bold tracking-wider text-gray-400 mb-1">PAYMENT TERMS</p>
            ...
            {!readonly && <button onClick={addItem} className="text-indigo-600 font-bold text-xs flex items-center gap-1 hover:underline print:hidden mb-3"><IconPlus className="w-3.5 h-3.5" />{lang === 'fa' ? 'افزودن ردیف' : 'Add row'}</button>}
            ...
            <div className="flex flex-col items-end gap-0 mb-6 text-[12px]">
              ...
              <div className="w-full flex justify-end border-b border-gray-100 py-2 items-center"><span className="text-gray-400 mr-6">VAT (<input type="number" className="w-10 text-center border-b border-gray-200 outline-none bg-transparent print:border-0" value={draft.taxRate} onChange={e => setField('taxRate', parseFloat(e.target.value) || 0)} />% - {draft.vatInclusive ? 'inclusive' : 'exclusive'}) ({cur})</span><span className="text-gray-500 w-28 text-right">{money(draft.taxAmount)}</span></div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border border-gray-200 rounded-md p-3 text-[11px] leading-relaxed">
                ...
                {!template.bankName && !template.iban && <span className="text-gray-300">{lang === 'fa' ? 'در «اطلاعات شرکت» تکمیل کنید' : 'Fill in Company Info'}</span>}
              </div>
              <div className="border border-gray-200 rounded-md p-3">
                <p className="text-[10px] font-bold tracking-wider text-gray-400 mb-1.5">NOTES / TERMS</p>

...
import { IconPaperclip, IconPlus, IconTrash, IconFile, IconUpload } from './Icons';
import {
...
export const TeamBrainstormPanel: React.FC<Props> = ({ currentUser, posts, lang }) => {
  ...
  const removePost = async (post: TeamBrainstormPost) => {
    ...
    if (detailId === post.id) setDetailId(null);
  };

  const truncate = (s: string, n: number) => (s.length <= n ? s : `${s.slice(0, n)}…`);

  const renderStickyCard = (post: TeamBrainstormPost, idx: number, onClick: () => void) => {
    const style = STICKY[post.color] || STICKY.yellow;
    ...
    return (
      <button
        ...
        type="button"
        onClick={onClick}
        className={`group relative text-right w-full min-h-[220px] p-5 pt-8 rounded-sm border border-black/10 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-200 ${style.card} ${rot}`}
      >
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {posts.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 px-1 py-2">
            {pagePosts.map((post, idx) => renderStickyCard(post, idx, () => setDetailId(post.id)))}
          </div>
          {totalPages > 1 && (
      ...
      {/* Compose modal */}
      {composeOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={resetCompose}>
      ...
      {/* Detail modal */}
      {detailPost && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 overflow-y-auto" onClick={() => { setDetailId(null); setReplyToId(null); setCommentText(''); setCommentFiles([]); }}>
          <div className={`relative w-full max-w-2xl my-6 p-6 pt-10 rounded-sm shadow-2xl border border-black/10 ${STICKY[detailPost.color].card}`} onClick={e => e.stopPropagation()}>
            <div className={`absolute -top-2 left-1/2 -translate-x-1/2 w-14 h-4 rounded-sm ${STICKY[detailPost.color].tape} border border-black/5`} />
            <div className="flex items-start justify-between gap-3 mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => setDetailId(null)} className="text-gray-500 hover:text-gray-800 text-xl leading-none px-2">×</button>
              </div>
            </div>

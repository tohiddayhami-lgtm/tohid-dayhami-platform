...
export const MetaShopView: React.FC<Props> = ({ shop, lang, onSubmitOrder, onSubmitReferral, onSubmitSupplierCollaboration, onLookup, embed }) => {
  ...
  // Anchor (base) price for strikethrough / revert reference.
  const anchorPrice = (p: MetaShopProduct, optId?: string): number => anchorUnitPrice(p, optId);
  // Current stored selling price (before temporary markup / discount).
  const sellingUnitPrice = (p: MetaShopProduct, optId?: string): number => {
  ...
  const listPackPrice = (p: MetaShopProduct): number | undefined => {
    const base = p.packPrice;
    if (base == null) return undefined;
    return markedUpPrice(shop, p, base) ?? base;
  };
...
.ms-price-amt { font-weight:800; font-size:15px; color:#0f172a; direction:ltr; unicode-bidi:isolate; }
.ms-price-amt.ms-pack { font-size:13px; color:#475569; font-weight:700; }
.ms-price-was, .ms-opt-was, .ms-citem-price, .ms-card-calc, .ms-fee-amt, .ms-disc-amt, .ms-inv-subtotal b, .ms-inv-total b, .ms-summary b { direction:ltr; unicode-bidi:isolate; }

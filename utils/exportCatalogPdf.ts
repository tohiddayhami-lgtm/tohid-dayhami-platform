...
function isNavyBand(el: HTMLElement): boolean {
  ...
  return el.innerHTML.includes('#0b1f3a') || el.innerHTML.includes('0b1f3a');
}
...
function groupUnits(body: HTMLElement): HTMLElement[][] {
  ...
  while (i < kids.length) {
    ...
    if (isNavyBand(el)) {
      ...
      while (i < kids.length && kids[i].tagName.toLowerCase() === 'div' && !isNavyBand(kids[i])) {
        unit.push(kids[i]);
        i++;
      }
...
function packPages(units: HTMLElement[][]): HTMLElement[][] {
  ...
  let cur: HTMLElement[] = [];
  let used = 0;
  const hOf = (els: HTMLElement[]) => els.reduce((s, el) => s + el.getBoundingClientRect().height + 6, 0);
...
export async function exportCatalogPdf(catalog: CompanyCatalog, filename: string): Promise<void> {
  const html = buildCatalogDocumentHtml(catalog, 'pdf');
...
/** Export from live preview element (WYSIWYG with on-screen preview). */
export { exportPdfFromPreviewElement } from './exportPreviewPdf';
